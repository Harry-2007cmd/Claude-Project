...
| D13 | **One shared color palette/typography sitewide** (dark background, purple `#7B5CFA` primary, teal `#2FD9C4` accent — from the Carpool reference), with **each feature keeping its own layout/structure** from its own reference image, re-skinned into the shared palette | Owner-provided references used 3 different color schemes (dark-purple Carpool, light Community, light red/orange Food); a single unified palette was chosen so the site doesn't look like 3 unrelated apps stitched together, while preserving the distinct layout ideas from each reference |
| D14 | Landing page = **Login page** (no marketing splash); **Community is the home screen** after auth; persistent Navbar = Community / Carpool / Food / Profile | Confirmed by owner |
| D15 | Added a **Profile** page to MVP scope (self-view only: info + own posts + own rides) | Confirmed by owner; reuses the existing `users` table plus one new nullable `bio` column — no new table needed |
